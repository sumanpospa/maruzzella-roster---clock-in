...
async function extractState() {
  ...
  try {
    ...
    console.log(`   - Current Week Shifts: ${state.rosters?.currentWeek ? Object.values(state.rosters.currentWeek).flat().length : 0}`);
    console.log(`   - Next Week Shifts: ${state.rosters?.nextWeek ? Object.values(state.rosters.nextWeek).flat().length : 0}`);
    ...
    if (state.timeLogs && state.timeLogs.length > 0) {
      ...
      const filename = `recovery-full-state-${new Date().toISOString().replace(/:/g, '-').split('.')[0]}.json`;
