      ...
      <div className="flex items-center gap-2">
        {isConfirmingDelete ? (
          ...
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity duration-200">
            <button
              onClick={onEdit}
              className="px-3 py-1 text-sm font-medium text-slate-700 bg-stone-100 hover:bg-stone-200 rounded-lg transition-colors"
            >
              Edit
            </button>
            <button
              ...
              title={
                isCurrentUser ? 'You cannot delete your own account.' : `Delete ${employee.name}`
              }
            >
              Delete
            </button>
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">
            Manage {currentUser.department} Staff
          </h2>
          <p className="text-stone-600">
            Add, edit, or remove staff members from {currentUser.department}.
          </p>
        </div>
