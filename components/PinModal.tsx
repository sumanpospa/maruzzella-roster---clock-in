...
const PinModal: React.FC<PinModalProps> = ({ isOpen, onClose, onSuccess, employee }) => {
  ...
  const keypadButtons = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', 'del'].map(
    (key, i) => {
    ...
    >
      ...
      >
        <div className="text-center">
          <h2 id="pin-modal-title" className="text-xl font-bold text-slate-800">
            Enter PIN
          </h2>
        ...
        <div className="flex justify-center items-center gap-3 mb-6 h-6">
          {error ? (
            <p className="text-red-600 text-sm font-medium animate-shake">{error}</p>
          ) : (
            pinDots
          )}
        </div>
        <div className="grid grid-cols-3 gap-2">{keypadButtons}</div>
      </div>
      <style>{`
