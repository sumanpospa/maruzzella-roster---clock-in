...
const RosterView: React.FC<RosterViewProps> = ({
  ...
  const handleShareWhatsApp = () => {
    ...
    try {
      ...
      link.download = `Maruzzella_Roster_${weekLabel.replace(' ', '_')}_${
  ...
  const handleCopyToNextWeek = () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to replace next week's roster with the current week's schedule? This action cannot be undone.",
      )
    ) {
      setRosters((prev) => ({
        ...prev,
            ...
            <button
              ...
              className={`px-4 py-1.5 text-sm font-semibold rounded-full transition-colors ${
                viewingWeek === 'currentWeek'
                  ...
                  : 'text-slate-600 hover:bg-stone-100/50'
              }`}
            >
              This Week
            </button>
              ...
              {kitchenEmployees.map((employee, index) => {
                ...
                return (
                  <tr key={employee.id} className="group border-b border-stone-200 last:border-b-0">
                    <td
                      className={`sticky left-0 p-3 z-20 transition-colors duration-150 border-r border-stone-300 shadow-[2px_0_4px_rgba(0,0,0,0.05)] w-[120px] ${rowBgClass} ${hoverBgClass}`}
                    >
                      ...
                      return (
                        <td
                          ...
                          onClick={() =>
                            ...
                            handleOpenModal(day, employee.id)
                          }
                          className={`relative p-2 align-top min-h-[80px] transition-colors duration-150 border-r border-stone-300 ${todayClass} ${hoverBgClass} ${
                            isManager && employeeShifts.length === 0
                              ? 'cursor-pointer hover:bg-orange-100/50'
                              : ''
                          }`}
                        >
                            ...
                            {employeeShifts.map((shift) => {
                              ...
                              return (
                                <div
                                  ...
                                  } ${COLORS[colorIndex]} transition-shadow`}
                                >
                                    ...
                                    <div className="flex -space-x-1.5 overflow-hidden mt-1.5">
                                      {otherEmployeesOnShift.map((otherEmp) => (
                                        <div
                                          key={otherEmp.id}
                                          title={otherEmp.name}
                                          className="inline-block h-5 w-5 rounded-full ring-1 ring-white/50 text-xs flex items-center justify-center bg-slate-500/50 text-white font-semibold"
                                        >
