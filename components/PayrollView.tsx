...
const StatusBadge: React.FC<{ status: TimeLog['status'] }> = ({ status }) => {
  switch (status) {
    ...
    case 'rejected':
      return (
        <span className="font-semibold px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs">
          Rejected
        </span>
      );
    case 'pending':
      return (
        <span className="font-semibold px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs">
          Pending
        </span>
      );
    default:
      return (
        <span className="font-semibold px-2 py-1 bg-sky-100 text-sky-700 rounded-full text-xs">
          Active
        </span>
      );
  }
...
const PayrollView: React.FC<PayrollViewProps> = ({
  ...
  const handleUpdateLogStatus = (logId: number, status: 'approved' | 'rejected') => {
    setTimeLogs((prevLogs) => prevLogs.map((log) => (log.id === logId ? { ...log, status } : log)));
  };
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-stone-600">
            {isManager
              ? 'Review, approve, and manage all recorded work hours for your department.'
              : 'View your work hours, approved time, and pending approvals.'}
          </p>
        </div>
        {isManager && (
          <button
      ...
      <div className="space-y-6">
        {employeePayData.map((data) => (
          <div
            key={data.id}
            className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-stone-200"
          >
                  ...
                  {data.pendingHours > 0 && (
                    <div className="text-left sm:text-right">
                      ...
                      <p className="text-2xl font-bold text-amber-500">
                        {formatDuration(data.pendingHours)}
                      </p>
                    </div>
                  )}
                  <div className="text-left sm:text-right">
                    ...
                    <p className="text-2xl font-bold text-orange-600">
                      {formatDuration(data.approvedHours)}
                    </p>
                  </div>
                </div>
                {isManager && (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.logs
                      ...
                      .map((log) => {
                        const duration = log.clockOutTime
                          ? log.clockOutTime.getTime() - log.clockInTime.getTime()
                          : null;
                        return (
                          <tr
                            key={log.id}
