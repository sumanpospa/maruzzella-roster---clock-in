...
async function restoreRecoveryData() {
  try {
    ...
    console.log(`   - Current Week Shifts: ${Object.values(recoveryState.rosters.currentWeek).flat().length}`);
    console.log(`   - Next Week Shifts: ${Object.values(recoveryState.rosters.nextWeek).flat().length}\n`);
    ...
    const currentWeekCount = currentState.rosters?.thisWeek ? Object.values(currentState.rosters.thisWeek).flat().length : 0;
    const nextWeekCount = currentState.rosters?.nextWeek ? Object.values(currentState.rosters.nextWeek).flat().length : 0;
    ...
    const finalCurrentWeekCount = mergedState.rosters?.thisWeek ? Object.values(mergedState.rosters.thisWeek).flat().length : 0;
    const finalNextWeekCount = mergedState.rosters?.nextWeek ? Object.values(mergedState.rosters.nextWeek).flat().length : 0;
    ...
    mergedState.timeLogs.forEach(log => {
      const date = new Date(log.clockInTime).toLocaleDateString('en-AU', { timeZone: 'Australia/Perth' });
    ...
    const backupFilename = `./backups/backup-post-recovery-${new Date().toISOString().replace(/:/g, '-').split('.')[0]}.json`;
